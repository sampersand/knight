# This begin block is just for examples.
BEGIN {
	source_code = " \
; = n 0 \
: WHILE < = n + n 1 100 \
...
function generate_ast(node_idx, token) {
	...
	if (substr(token, 1, 1) != "f" || token ~ /f[TFNP]/)
		return node_idx
...
function to_string(input, dontrun) {
	...
	die("bad input for 'to_string': '" input "'")
}
...
function to_number(input, dontrun) {
	...
	die("bad input for 'to_number': '" input "'")
}
...
function to_boolean(input, dontrun) {
	...
	die("bad input for 'to_boolean': '" input "'")
}
...
function run(node_idx, token, fn, tmp, arg1, arg2, arg3) {
	if (!(node_idx in ast))
		die("ice: token number '" node_idx "' doesn't exist in the ast")
