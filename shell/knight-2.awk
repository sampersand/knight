...
function arity(token) {
	if (substr(token, 1, 1) !~ /^f/) return 0
	if (token ~ /f[FTNPR]/) return 0
	if (token ~ /f[OEBCQ`!L]/) return 1
	if (token ~ /f[-+*\/%^?<>;=W&]/) return 2
	if (token ~ /f[IG]/) return 3
	if (token ~ /f[S]/) return 4

	die("unknown token '" token "' encountered.")
}

# Generates an AST tree, storing values in the `ast` global variable. Returns
# the index of the node that was just generated.
# (all parameters are simply local variables)
function generate_ast(node_idx, token, i) {
	if (!(token = next_token())) {
		return
	}
	...
	for (i = 0 ; i < arity(token); i++) {
		if (!(ast[node_idx, i] = generate_ast())) {
			die("missing argument #" idx " for function '" substr(token, 2, 1) "'")
		}
	}
...
function run(node_idx, token, fn, tmp, arg1, arg2, arg3) {
	...
	# Randomly pick an integer from arg1 thru arg2 and return it.
	if (fn == "R") {
		arg1 = to_number(arg1)
		arg2 = to_number(arg2)
		return "n" int(rand() * (arg2 - arg1) + arg1)
	}
	...
	die("unknown function to evaluate '" fn "'")
}
